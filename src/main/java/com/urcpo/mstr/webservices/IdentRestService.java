...
import com.urcpo.mstr.services.IdentService;
    ...
    public Response get(String credentials) {
        ...
        try {
            ...
            WebResource webResource = client
                    .resource(appServ + "/mstr/rest/ident/admin");
            client.addFilter(new HTTPBasicAuthFilter(username, password));
            UserEnum type;

            // Get the protected web page:
            webResource = client.resource(appServ + "/mstr/rest/ident/admin");
            log.info("BABAR2!");
            ClientResponse response = webResource.get(ClientResponse.class);
            if (response.getStatus() != 200) {// pas admin...
                webResource = client.resource(appServ + "/mstr/rest/ident/teacher");
                response = webResource.get(ClientResponse.class);
                if (response.getStatus() != 200) {// pas teacher...
                    webResource = client.resource(appServ + "/mstr/rest/ident/student");
                    response = webResource.get(ClientResponse.class);
                    if (response.getStatus() != 200) {// pas student...
                        webResource = client.resource(appServ + "/mstr/rest/ident/guest");
                        response = webResource.get(ClientResponse.class);
                        if (response.getStatus() != 200) {// pas teacher...

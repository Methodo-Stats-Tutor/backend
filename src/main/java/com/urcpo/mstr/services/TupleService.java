...
import static com.urcpo.mstr.servlets.LoadOnto.ontologie;
import com.urcpo.mstr.owlgraphexport.GraphFormater;
import com.urcpo.mstr.owlgraphexport.GraphGenRoot;
...
public class TupleService {
    ...
    public String getOntoCytoGraph(String usrId) throws FileNotFoundException {
        ...
        String queryBegin
                ...
                + "PREFIX owl:      <http://www.w3.org/2002/07/owl#> \n"
                + "PREFIX sparqldl: <http://pellet.owldl.com/ns/sdle#>\n "
                + "CONSTRUCT {?USR mstr:masterNotion ?NOTIONBIND}\n"
                + "WHERE {";
        String queryEnd = "}";

        String query
                = queryBegin
                + "?USR mstr:masterNotion ?NOTION  .\n"
                + "BIND(IRI(str(?NOTION)) as ?NOTIONBIND) .\n"
                + "?USR a mstr:Student \n"
                + "FILTER (?USR = mstr:" + usrId + ")\n"
                + queryEnd;
